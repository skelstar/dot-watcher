...
- **This build has no SQLite fallback** — `SessionStore` requires `ConnectionString` to point at a reachable Postgres instance to start at all. Staging and Production were cut over to one shared Postgres instance on 2026-07-22 (see [`.ai/plans/shared-database-cutover.md`](../.ai/plans/shared-database-cutover.md) for the full rollout history) — both environments' `dot-watcher-server-secrets` now include `ConnectionString`, wired into each Deployment's pod spec as an explicit `secretKeyRef` env var (this had to be added manually via `kubectl patch`; `server/.deploy.yaml` doesn't express it yet — see the plan doc's follow-up note before doing a from-scratch redeploy).
