...
- Licence: CC BY 4.0. The product must visibly show attribution and link to the LINZ copyright statement and the Basemap's custom attribution text. The mapping library does not do this automatically. **Confirmed directly by LINZ (email to the human, 2026-09-21):** "you are correct we just ask to be attributed" — validates the Phase 3 approach as-is, no further requirement.
...
- **Offline/poor-coverage option, not built here — flagged as a follow-up idea (2026-09-21, from LINZ's key-assignment email):** LINZ explicitly suggests MBTiles for users "likely to be in areas without coverage" — directly relevant to this app, since trail runners are often exactly that. Download a complete offline tileset: `https://basemaps.linz.govt.nz/v1/export/topographic-v2/WebMercatorQuad.mbtiles?api=API_KEY`. LINZ's own update cadence: raster/vector tiles refresh weekly, topographic *data* quarterly — LINZ recommends refetching an exported MBTiles file roughly every 6 months. Out of scope for this plan (only the map library/style is in scope — see Goal), but worth a future plan of its own if patchy-connectivity trail use turns out to matter in practice (see "Risks and notes" below, already flagged this as worth a runbook note).
- Reference links from LINZ: general vector-tiles docs at https://basemaps.linz.govt.nz/docs/user-guide/working-with-vector-tiles/, and a live style preview at https://basemaps.linz.govt.nz/@-41.8899962,174.0492437,z5?style=topographic-v2&i=topographic-v2&labels=true (useful to eyeball against this app's own rendering during Phase 5).
...
- **Follow-up idea, not built here (2026-09-21):** trail runners are exactly the "areas without coverage" case LINZ's own key-assignment email flags as an MBTiles use case (see Facts). This app currently only fetches live tiles over the network — a runner out of signal on a trail gets no basemap at all. Worth a dedicated plan later if that turns out to matter in practice: pre-fetch/cache the session's route bounding box (echoes a similar open question already flagged in `.ai/plans/mapbox-terrain-tiles.md` for the iOS app's `MKTileOverlay` caching).
- **Follow-up idea, not built here (2026-09-22): 3D terrain.** LINZ Basemaps has a "3D" feature (the mountain icon at basemaps.linz.govt.nz) — draping topo/aerial imagery over their 1m DEM via `map.setTerrain({ source, exaggeration })` and a pitched camera. `topographic-v2`'s style JSON already declares the needed raster-dem sources (`LINZ-Terrain`, `LINZ-Elevation-Hillshade`); it's opt-in (`setTerrain` is never called), so nothing here does this today.
  - Runner `Marker`s are terrain-aware by design — MapLibre re-projects from the real lng/lat on every transform change and clamps to ground elevation, so dots land in the geographically correct 3D spot, not floating at sea level.
  - **But not safe to build on the currently-installed `maplibre-gl@6.10.0` yet.** Checked `maplibre-gl-js` GitHub directly: fix `#8433` ("markers and popups follow the terrain and the projection after the map settles") merged 2026-09-18 — 4 days before this note — and has **not shipped in any tagged release** (`6.10.0` is still latest). Their own testing found markers could be offset by up to ~1,337px right after terrain tiles load or a projection/terrain transition, until the map fires `idle`. Directly relevant here: this app moves the camera and updates marker positions constantly (auto-fit, follow, ~15s live polling), so this bug's trigger conditions would recur often. Re-check once a release containing `#8433` ships.
  - **Update 2026-09-25: experiment started.** `#8433` shipped in `maplibre-gl@6.11.0` (2026-09-22); upgraded to `^6.11.2`. Added an experimental `Terrain3DToggle` button (top-right, below the style toggle) that calls `map.setTerrain({ source: 'LINZ-Terrain', exaggeration: 1 })` and eases pitch to 60° (back to 0 when off); terrain is re-applied on `style.load` so it survives the topo/aerial toggle. Not yet checked on-device — look specifically for marker offset after terrain loads, auto-fit/follow framing at pitch, and how runners at different elevations read.
  - Two non-bug UX considerations if this gets picked up later: a pitched 3D camera makes the existing 2D bounding-box `fitBounds` auto-fit math less reliable for framing; and runners at different elevations will appear vertically offset from each other on screen even when correctly positioned, which may or may not read well for a group of dots.
...
- 2026-09-21, Claude Code (client-linz-maps branch), Human shared LINZ's key-assignment email and asked whether anything in it adds value to the plan. Checked whether the assigned "unrestricted" key differed from what's in `client/.env` — it already matched (`client/.env` had already been updated), no action needed there. Pulled genuinely new value from the email into Facts and Risks/notes: LINZ's own suggestion to use MBTiles for offline/poor-coverage caching (flagged as a real follow-up idea, not built now — directly relevant since trail runners are the exact "areas without coverage" case, and echoes the same open caching question already sitting in `.ai/plans/mapbox-terrain-tiles.md` for the iOS side), the update cadence they gave (tiles weekly, topo data quarterly, ~6-monthly MBTiles refetch), written confirmation that the attribution approach (Phase 3) is correct, and reference links (docs, a live style-preview URL useful for Phase 5). Human said they'll open the PR themselves on this machine. What's next: nothing further from here unless asked — Phase 5, then the human's own PR.
- 2026-09-22, Claude Code (client-linz-maps branch), Human asked whether runner dots would still appear correctly with LINZ's 3D terrain feature. Researched before answering: confirmed LINZ's "3D" is `map.setTerrain()` over their 1m DEM (sources already present but unused in `topographic-v2`'s style JSON), that MapLibre `Marker`s are terrain-aware by design, but also found a live, version-specific problem worth flagging rather than glossing over — `maplibre-gl-js` fix `#8433` (markers/popups can be up to ~1,337px offset after terrain loads or a projection/terrain transition, until `idle`) merged 2026-09-18 and has not shipped in any release yet, so it's present in our installed `6.10.0`. Added as a new follow-up idea in Risks/notes (not built, needs that fix to ship first) alongside the two non-bug UX considerations (pitched-camera auto-fit, vertically-offset runners at different elevations). What's next: nothing further from here unless asked — human is committing this note and will push themselves.
